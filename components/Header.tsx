...
export default function Header() {
  ...
  return (
    <>
      ...
      <header
        className={`sticky top-0 z-50 bg-aura-white transition-shadow duration-300 ${
      ...
      >
        ...
        <div className="border-b border-aura-border">
          <div className="max-w-screen-xl mx-auto px-4 md:px-8 h-16 flex items-center justify-between">
            ...
            <Link href="/" className="absolute left-1/2 -translate-x-1/2 md:static md:translate-x-0">
              <span className="font-serif text-2xl tracking-[0.1em] text-aura-black uppercase">
                Aura
              </span>
            </Link>
            ...
            <nav className="hidden md:flex items-center gap-8 flex-1 justify-center" onMouseLeave={handleMouseLeave}>
              {NAV_ITEMS.map((item) => (
                <div
                  key={item.label}
                  className="relative"
                  onMouseEnter={() => item.mega ? handleMouseEnter(item.label) : setActiveMenu(null)}
                >
                  <Link
                    href={item.href}
                    className={`nav-link flex items-center gap-1 ${
                      item.label === 'Sale' ? 'text-aura-accent' : ''
                    }`}
                  >
                    {item.label}
            ...
            <div className="flex items-center gap-3">
              ...
              <Link href="/account" className="p-2 hover:text-aura-gold transition-colors hidden md:block">
                <User size={18} />
              </Link>
              <button
            ...
            <div className="max-w-screen-xl mx-auto px-8 py-10 grid grid-cols-4 gap-8">
              {item.mega?.sections.map((section) => (
                <div key={section.title}>
                  <h4 className="text-[10px] tracking-[0.25em] uppercase font-body text-aura-gold mb-4">
                    {section.title}
                  </h4>
                  <ul className="space-y-2">
                    {section.links.map((link) => (
                      <li key={link.label}>
                        <Link
                          href={link.href}
                          className="text-sm font-body text-aura-charcoal hover:text-aura-black hover:tracking-wide transition-all duration-200"
        ...
        {activeMenu && (
          <div
            className="fixed inset-0 top-[108px] bg-black/20 z-40"
          ...
          <nav className="p-6">
            {NAV_ITEMS.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className={`block py-4 border-b border-aura-border text-sm tracking-[0.15em] uppercase font-body ${
                  item.label === 'Sale' ? 'text-aura-accent' : 'text-aura-charcoal'
                }`}
      ...
      {/* Overlay */}
      <div
        className={`overlay ${cartOpen || searchOpen ? 'overlay-visible' : ''}`}
